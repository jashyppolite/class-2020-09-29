...
```{r}
d_2019 <- read_xlsx("raw_data/class_enrollment_summary_by_term_12.20.19.xlsx",
                    skip = 3) %>%
  clean_names() %>%
...
#  read_file(file_location = "raw_data/class_enrollment_summary_by_term_10.24.18.xlsx")
#  read_file(file_location = "raw_data/class_enrollment_summary_by_term_10.20.17.xlsx")
#  read_file(file_location = "raw_data/class_enrollment_summary_by_term_fall_2016.xlsx")
#  read_file(file_location = "raw_data/class_enrollment_summary_by_term_fall_2015.xlsx")
# d_2019_take_2
```
...
```{r}
# will be useful over time to see and use diff spreadsheets, especially over time
read_enrollment <- function(file, skip_lines = 3) {
  
  read_xlsx(file, skip = skip_lines) %>%
  clean_names() %>%
  filter(! is.na(course_title)) %>%
  select( -instructor_full_name, -course_section_code, -(grad:total)) %>%
  rename_with(~str_replace(.x, "course_", "")) %>%
...
d_2019 <- 
  read_enrollment(file = "raw_data/class_enrollment_summary_by_term_12.20.19.xlsx", skip_lines = 3)
d_2018 <-
  read_enrollment(file = "raw_data/class_enrollment_summary_by_term_10.24.18.xlsx", skip_lines = 2)
d_2017 <- 
  read_enrollment(file = "raw_data/class_enrollment_summary_by_term_10.20.17.xlsx", skip_lines = 3)
d_2016 <- 
  read_enrollment(file = "raw_data/class_enrollment_summary_by_term_fall_2016.xlsx", skip_lines = 0)
d_2015 <- 
  read_enrollment(file = "raw_data/class_enrollment_summary_by_term_fall_2015.xlsx", skip_lines = 3)
...
x <-  bind_rows(list(d_2015, d_2016, d_2017, d_2018, d_2019), .id = "year") %>%
  #year was a character, so we made it a number
...
```{r}
ggplot(x, aes(x = year, y = u_grad, fill = department)) + geom_col() + 
  theme_linedraw()
```
